...
{
    public class WorkerManager : MainManager
    {
        public WorkerManager(IApplicationDbContext _appCnx) : base(_appCnx)
        {
        }

        public WorkerManager(IApplicationDbContext _appCnx, ILog _log) : base(_appCnx, _log)
        {
        }
                        ...
                        foreach (EPU.Empleado eEmpleado in lstEmpleados)
                        {
                            ...
                            if (eEmpleado.listaTurno.Count > 0)
                            {
                                foreach (EPU.Turno eTurno in eEmpleado.listaTurno)
                                {
                                    ...
                                    //eMarcacionXDefecto.fecha_marcacion = DateTime.Now;
